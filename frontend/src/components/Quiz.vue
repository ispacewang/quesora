        ...
        <Transition name="question-change" mode="out-in">
          <div :key="idx" class="question-wrapper">
            ...
            <div style="margin-bottom: 16px;">
              <el-tag type="primary" style="margin-right:8px;">{{ question.type }}</el-tag>
              <el-tag v-if="question.meta?.['题目分类']" type="success" style="margin-right:8px;">{{ question.meta['题目分类']
              }}</el-tag>
              <el-tag v-if="question.meta?.['一级纲要']" type="warning">{{ question.meta['一级纲要'] }}</el-tag>
            </div>
            <div style="margin-bottom: 24px; text-align: left; font-size: 1.1em; line-height: 1.6;">{{ question.question
            }}</div>
            <el-input v-if="isShortAnswer" v-model="userAnswer" type="textarea" :rows="4" placeholder="请输入你的答案"
              style="margin-bottom: 16px;" />
            <el-checkbox-group v-else-if="isMultiChoice" v-model="userAnswer" class="option-group">
              <el-checkbox v-for="(opt, i) in question.options" :key="i" :label="String.fromCharCode(65 + i)"
                class="duoxuan">
                {{ String.fromCharCode(65 + i) + '. ' + opt }}
              </el-checkbox>
            </el-checkbox-group>
            <el-radio-group v-else v-model="userAnswer" class="option-group">
              <el-radio v-for="(opt, i) in question.options" :key="i"
                :label="isJudge ? opt : String.fromCharCode(65 + i)" class="danxuan">
            ...
            <div style="margin-top: 24px;">
              <div class="button-group">
                <el-button class="submit-button" type="primary" size="large" round :icon="Check" @click="submit"
                  :disabled="(isMultiChoice ? userAnswer.length === 0 : !userAnswer) || submitted || submitting"
                  :loading="submitting">
                  {{ submitting ? '判题中...' : '提交' }}
                </el-button>
                <!-- 【改】修改 v-if 条件，在错题库模式下答对或答错后都显示“下一题” -->
                <el-button v-if="submitted && (resultType === 'error' || isMistakeMode)" class="next-button"
                  size="large" round :icon="ArrowRight" @click="next">
                  下一题
                </el-button>
              </div>
            </div>
            ...
            <div v-if="submitted" style="margin-top: 24px; text-align: left;">
              ...
              <div v-if="explanation"
                style="margin-top: 16px; padding: 16px; background-color: #f7f8fa; border-radius: 8px;">
                <strong style="color: #303133;">【深度解析】</strong>
                <p style="margin: 8px 0 0 0; color: #606266; line-height: 1.7;">{{ explanation }}</p>
              </div>
            </div>
          </div>
        </Transition>
...
const isJudge = computed(() => {
  ...
  return opts.length === 2 && ((opts[0] === '正确' && opts[1] === '错误') || (opts[0] === '对' && opts[1] === '错'));
});
...
const loadFromApi = async () => {
  try {
    const res = await getQuestion(currentBank.value);
...
const isCorrect = formatAnswer(userAnswer.value) === formatAnswer(correctAnswer);
...
if (isCorrect) {
  ...
  // 【修改】发送结构统一的数据
  emit('answer-submitted', { isCorrect: true, questionData: questionDataForEmit });
} else {
  resultType.value = 'error';
  resultTitle.value = `回答错误，正确答案：${formatAnswer(correctAnswer)}`;
  explanation.value = question.value.explanation;
  // 【修改】发送结构统一的数据
  emit('answer-submitted', { isCorrect: false, questionData: questionDataForEmit });
}
...
const resetQuestion = () => { loadQuestion(); };
